...
function noisy(f) {
  return (...args) => {
    ...
    console.log(args);
    let result = f(...args);
    console.log(`Called with ${args} and returning ${result}`);
    return result;
  };
}

holder = noisy(Math.min);
console.log(holder(12, 34, 5));

// Can provide new forms of flow control
...
// )
//   module.exports = utfScripts;
// if (typeof global != "undefined" && !global.utsScripts)
//   global.utfScripts = utfScripts;

let test = utfScripts.filter((script) => {
  return !script.living;
});

let rtol = utfScripts.filter((script) => {
  return script.direction === "rtl";
});

let ttob = utfScripts.filter((script) => {
  return script.direction === "ttb";
});

console.log(rtol.map((script) => script.name));
console.log(ttob.map((script) => script.name));

// find language with the most characters
function characterCount(script) {
  return script.ranges.reduce((totalCount, [from, to]) => {
    return (totalCount += to - from);
  }, 0);
}

console.log(
  utfScripts.reduce((currMax, script) => {
    return characterCount(currMax) < characterCount(script) ? script : currMax;
  })
);

// Exercises

// flattening array of arrays to a single array.

// ME - this is really neat to be able to use reduce with arrays and starting the
// cumulative value with the empty array.
function flatten(inputNestedArray) {
  const outputFlatArray = inputNestedArray.reduce((flattened, current) => {
    console.log(current);
    flattened = flattened.concat(current);
    return flattened;
  }, []);
  return outputFlatArray;
}

// own loop implementation

// ME - very neat use of less conventional for loop structure that shows how extendable it is
// with having it use callback functions for the increment test and updates.
...
// everything - returns true if all array elements are true

function every(inputArray, testFunction) {
...
// alternative using 'some' method
function everyWithSome(inputArray, testFunction) {
  return !inputArray.some((element) => !testFunction(element));
}

// compute dominant writing direction in a string of text

// Find the script given the character code
function characterScript(code) {
  for (let script of utfScripts) {
    if (
      script.ranges.some(([from, to]) => {
        // test for 'some' to see if code is in the current char range for the looped script
...
function dominantDirection(text) {
  let characterCounts = countBy(text, (char) => {
  ...
  let maxName = "";
  let maxValue = 0;
  characterCounts.map((characterEntry) => {
    if (characterEntry.count > maxValue) {
      maxValue = characterEntry.count;
      maxName = characterEntry.name;
    }
  });
